                ...
                .map(async legal => {
                    ...
                    if(parseInt(crossData.from) === parseInt(process.env.CHAIN_ID)) {
                        ...
                        request(options)
                            ...
                            .catch((error) => {
                                console.log("[ERROR] Deploy to relay chain fail. Error: " + error.message || error.error );
                            })
                    }
                    ...
                    if(parseInt(crossData.to) === parseInt(process.env.CHAIN_ID)) {
                        ...
                        .catch((error) => {
                            console.log("[ERROR] Reply to relay chain fail. Error: " + error.message || error.error );
                        })
                    }
            ...
            this.queue.filter(group => new Date().getTime() > new Date(group.createTime).getTime() + (process.env.MINUTES_TIME_FOR_REVOKE * 60 * 1000) )
                .map(async groupNeedsRemove => {
                    try {
                        ...
                        await this.manager.revokeTransaction(groupNeedsRemove.voteFor);
                        this.remove(groupNeedsRemove.voteFor);
                    } catch (error) {
                        console.log("[ERROR] change state to revoke for " + groupNeedsRemove.voteFor + " error, Error: " + error.message);
                    }
                });
        }, process.env.MINUTES_TIME_FOR_CHECK * 60 * 1000);
    }
